...
class Analyze:
    ...
    def process_job_result(self, job_result: JobData, gathered_occurrences: dict = None):
        try:
    ...
    def __write_request_info_to_report(self, gathered_occurrences):
        ...
        pandasutil.append_df_to_excel(self.report_file_path, request_settings,
            sheet_name=analysis_report.REQUEST_INFO_SHEET_NAME)

        # write a gap for occurrence
        gaps = pd.DataFrame([{"": ""}, {"": ""}])
        pandasutil.append_df_to_excel(self.report_file_path, gaps,
            sheet_name=analysis_report.REQUEST_INFO_SHEET_NAME, header=True)
        ...
        pandasutil.append_df_to_excel(self.report_file_path, occurrence_report,
            sheet_name=analysis_report.REQUEST_INFO_SHEET_NAME, header=True)
