...
class AsremlAnalyze(Analyze):
    ...
    def pre_process(self):
        ...
        job = self._get_new_job(job_name, job_status, status_message)

        try:
            job_input_files = self.get_process_data(self.analysis_request).run()
            self._update_job(job, "IN-PROGRESS", "Data Pre-Processing completed.")
            return job_input_files
        except (DataReaderException, DpoException) as e:
            self.analysis.status = "FAILED"
            self._update_job(job, "FAILED", "Failed during Data Pre-Processing.")
    ...
    def run_job(self, job_data, analysis_engine=None):
        ...
        job = self._get_new_job(job_data.job_name, job_status, status_message)

        try:
            cmd = [analysis_engine, job_data.job_file, job_data.data_file]
            _ = subprocess.run(cmd, capture_output=True)
            job = self._update_job(job, "COMPLETED", "Completed the job.")
            ...
            self.analysis.status = "FAILED"
            self._update_job(job, "FAILED", str(e))
            raise AnalysisError(str(e))
        finally:
            self.db_session.commit()
    ...
    def process_job_result(self, job_result: JobData, gathered_objects: dict = None):
        try:
            ...
            self._update_job(job, "SUCCESS", "Asreml analysis completed successfully")
