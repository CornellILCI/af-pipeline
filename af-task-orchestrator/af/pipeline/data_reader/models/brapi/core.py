...
class Context(BaseModel):
    pydantic.RootModel: List[AnyUrl] = Field(
        ...,
...
class GeometryItem(BaseModel):
    ...
    type: Literal['Point'] = Field(..., description='The literal string "Point"', example='Point')#I think all the literals have to be 'Literal[thing]' now? So says the error message


class Coordinate(BaseModel):
    pydantic.RootModel: List[Any] = Field(
...
class GeoJSON1(BaseModel):
    ...
    ..., description='The literal string "Feature"', example='Feature',
    )#Pydantic 2 hates 'mapping' nested in discriminator property key, so making this not optional... -JDLS
...
class GeometryItem2(BaseModel):
    ...
    type: Literal['Point'] = Field(..., description='The literal string "Point"', example='Point')
...
class LinearRing(BaseModel):
    pydantic.RootModel: List[LinearRingItem] = Field(
        ...,
...
class Polygon(BaseModel):
    pydantic.RootModel: List[List[PolygonItem]] = Field(
        ...,
        description='An array of linear rings',
        example=[
            [
...
class Position(BaseModel):
    pydantic.RootModel: List[float] = Field(
        ...,
        description='A single position',
        example=[-76.506042, 42.417373, 123],
        min_items=2,
...
class ExternalReferences(BaseModel):
    pydantic.RootModel: List[ExternalReferencesInner] = Field(
        ...,
        description='An array of external reference ids. These are references to this piece of data in an external system. Could be a simple string or a URI.',
        example=[
            {'referenceId': 'doi:10.155454/12341234', 'referenceSource': 'DOI'},
...
class ListResponse(BaseModel):
    field_context: Optional[Context] = Field(None, alias='@context')
    metadata: Metadata 
    result: ListData #JDLS - make this a list?
    
class ListData(BaseModel, arbitrary_types_allowed=True,extra='allow'):
    data: list[Dict[str,str|Dict[str,str|Dict[str,str|None]|List[Dict[str,str|None]]|None]|List[str]|None]|None] = Field(None)#Okay - lets just make it a list of strings - doubly nested Data -> observationunitposition-> observation level
...
class ListDetails(BaseModel):
    ...
    data: Optional[List['str']] = Field( #JDLS - trying a thing
        None,
        description='The array of DbIds of the BrAPI objects contained in a List',
        example=['758a78c0', '2c78f9ee'],
    ) 
    ...
    listDbId: Optional[str] = Field(
        None, description='The unique identifier for a List', example='6f621cfa'
    )
    listDescription: Optional[str] = Field(
        None,
        description='Description of a List',
    ...
    listType: Optional[ListType1] = Field(
        None,
        description='A flag to indicate the type of objects that are referenced in a List',
        example='germplasm',
    )
