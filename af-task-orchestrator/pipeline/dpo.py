...
class ProcessData:
    ...
    def seml(self):
        ...
        traits: list[Trait] = self._get_traits()
        ...
        for occurrence_id in occurrence_ids:
            ...
            for trait in traits:
                ...
                plots_and_measurements = self._format_result_data(plots_and_measurements, traits)

                if not plots_and_measurements.empty:
                    job_input_files = self._write_job_input_files(
                        f"{occurrence_id}_{trait.trait_id}",
                        plots_and_measurements,
                        trait)
                    yield job_input_files
    ...
    def _format_result_data(self, plots_and_measurements, trait):
        ...
        plots_and_measurements[["trait_value"]] = (
            plots_and_measurements[["trait_value"]].fillna(config.UNIVERSAL_UNKNOWN))
    ...
    def _get_asreml_option(self, analysis_config_id: str):
        asreml_options = services.get_analysis_config_properties(
            self.db_session, analysis_config_id, "asrmel_options")
...
#########
    ...
    def run(self):
        ...
        elif exptloc_analysis_pattern == "SEML":
