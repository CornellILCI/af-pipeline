...
#reads JSON
jsonInput <- fromJSON(content = args[1])
...
#           "data=inputData)",
#           sep = "")
jsonInput$formula <- gsub("%trait%", jsonInput$trait, jsonInput$formula)
...
#sln
BVs <- as.data.frame(mix1[["U"]][["ID"]][[jsonInput$trait]])
colnames(BVs) <- "breeding.value"
BVs.std.error <- sqrt(diag((mix1[["PevU"]][["ID"]][[jsonInput$trait]])))
