...
parser.add_argument("N", type=int, help="Number of tests to generate.")
parser.add_argument(
    "-m",
    "--mode",
    choices=["auto", "manual"],
...
parser.add_argument("-p", "--port", type=int, default=80, help="default is 80.")
...
while i < numTests:
    ...
    if os.path.exists(jcfTemplate) and os.path.exists(lstTemplate):
        ...
        if args.mode == "auto":
            ...
            reqPath = simbaUtils.cfg["int"] + "/" + reqID + "/" + req
        else:
            reqPath = reqID + "/" + req
        ...
        if args.mode == "auto":
            # copy request file to input folder
            jcfPath = simbaUtils.cfg["int"] + "/" + reqID + "/" + jcf
        else:
            jcfPath = reqID + "/" + jcf
        ...
        if args.mode == "auto":
            ...
            lstPath = simbaUtils.cfg["int"] + "/" + reqID + "/" + lst
        else:
            lstPath = reqID + "/" + lst
...
if args.mode == "auto":
    ...
    for input in inputFolders:
        ...
        cmd = "curl -X POST " + "http://localhost:{}/v1/randomize/".format(args.port) + input
        os.system(cmd)
        time.sleep(1)
    ...
    for input in inputFolders:
        ...
        print("Service response:")
        cmd = "curl -X GET " + "http://localhost:{}/v1/status/".format(args.port) + input
        os.system(cmd)
        msg = '\nIf response is "queued", manually check ' + "status with:"
        print(msg)
        print(cmd)
        time.sleep(1)
