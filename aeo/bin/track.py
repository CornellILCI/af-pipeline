...
parser.add_argument("reqID", type=str, help="request to track")
parser.add_argument(
    "-m",
    "--mode",
    choices=["new", "update", "addJob", "updateJob"],
    help="new (new reqID), \
                          update (update status of reqID), \
                          addJob (add job to reqID), \
                          updateJob (update status of job)",
)
parser.add_argument("-s", "--status", choices=["queued", "failed"], help="status of reqID: queued or failed")
parser.add_argument("-j", "--jobID", type=str, help="id of job to add/update")
parser.add_argument(
    "-p",
    "--parent",
    type=str,
...
if args.reqID:
    if args.mode == "new":
        if args.status:
            ...
            # get req path
            reqPath = simbaUtils.cfg["int"] + "/" + args.reqID + "/" + args.reqID + ".req"
            if os.path.exists(reqPath):
                simbaUtils.genShaFile(reqPath)
                sha = simbaUtils.strSha
                dbUtils.addAnalysis(args.reqID, sha, reqType, args.status)
            else:
