...
try:
  with open(reqFile, 'r') as j:
    ...
    if obj['parameters']['trait_analysis_pattern'] != 2:
      ...
      # Get number of jobs:
      simbaUtils.getNjobs(expLocAnPat,trtAnPat,
                          nExp,nOcc,nTrt,nRespVar)
      ...
      trackOptAdd=args.dir + " -m addJob -j " + \
        dpoJobID + " -p " + dpoJobID
      trackOptUpd=args.dir + " -m updateJob -j " + \
        dpoJobID
      ...
      while (i <= nJobs):
        ...
        if trackOn:
          run=run + track + " " + trackOptAdd + "\n" + \
              engine + " " + reqAs + " " +  reqCsv +  " " + \
              outFile + " " + errFile + "\n" + \
              track + " " + trackOptUpd + "\n"
        else:
          run=run + engine + " " + reqAs + " " +  reqCsv + \
              " " + outFile + " " + errFile + "\n"
        i=i+1
