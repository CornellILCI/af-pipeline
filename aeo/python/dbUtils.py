...
def createConn():
    getDBcredentials()
    connStr = "host=" + dbHost + " port=" + dbPort + " dbname=" + dbName + " user=" + dbUser + " password=" + dbPswd
...
def updateAnalysis(analysisId):
    ...
    sql = "SELECT time_end, status " + "FROM job " + "WHERE analysis_id='{0}'".format(analysisId)
...
def getAnalysisId(qry, is_sha):
    ...
    if is_sha:
        sql = "SELECT id " + "FROM analysis " + "WHERE sha='{0}'".format(qry)
    else:
        sql = "SELECT id " + "FROM analysis " + "WHERE request_id='{0}'".format(qry)
...
def getJobId(qry):
    ...
    sql = "SELECT id " + "FROM job " + "WHERE job.name='{0}'".format(qry)
...
def getStatusAnalysis(qry, is_sha):
    ...
    if analysisId:
        ...
        if is_sha:
            sql = "SELECT status, msg " + "FROM analysis " + "WHERE sha='{0}' ".format(qry)
        else:
            sql = "SELECT status, msg " + "FROM analysis " + "WHERE request_id='{0}' ".format(qry)
