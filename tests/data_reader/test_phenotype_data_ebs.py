...
def get_plot_data_response():
    ...
    return read_mock_json_file(
        "tests/data_reader/plot_data_mock_response.json")
...
class TestPhenotypeDataEbs(TestCase):
    ...
    def test_get_plot_measurements(self, mock_post):
        ...
        plot_measurements_result = PhenotypeDataEbs(
            api_base_url="http://test").get_plot_measurements("testid")

        # assert dataframe is returned
        assert isinstance(plot_measurements_result, pd.DataFrame)

        plot_measurements_result = plot_measurements_result[
            plot_measurements_expected.columns]
