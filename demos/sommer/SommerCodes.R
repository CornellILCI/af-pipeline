...
#for multitraits
cat("Running multi-trait model...\n-->Formula: cbind(Yield, NDVI1)~Rep\n")
...
results_mix2.covariances <- summary(mix2)$varcomp #this is how you access the (co)variances
results_mix2.GEBV <- as.data.frame(mix2$U$`u:ID`$Yield) #and this is how you access the breeding values

write.csv(results_mix2.covariances , "output/results_mix2.covariances.csv")
write.csv(results_mix2.GEBV , "output/results_mix2.breeding_values.csv")
