...
stat_factor_meta AS (
	...
	VALUES(
		'definition', 'trialDbId', (SELECT id FROM stat_factor)
	)
),
...
stat_factor_meta AS (
	INSERT INTO af.property_meta(code,value,property_id) 
	VALUES(
		'definition', 'positionCoordinateY', (SELECT id FROM stat_factor)
	)
),
stat_factor_config_10001_link AS (
	INSERT INTO af.property_config (
...
WITH config_formula AS (
	INSERT INTO af.property(
		...
		'Univariate. Replicate as fixed and genotype as random effect', 
		'fixed = {trait_name} ~ rep, random = ~ genotype'
	) RETURNING id
),
...
WITH config_formula AS (
	INSERT INTO af.property(
		...
		'GBLUP univariate. Replicate as fixed effect.', 
		'fixed = {trait_name} ~ rep, random = ~ vs(genotype, Gu=A)'
	) RETURNING id
),
config_formula_property_config_link AS (
	INSERT INTO af.property_config(
...
WITH config_prediction AS (
	INSERT INTO af.property(
		...
		'catalog_item', 'character varying', 'g', 'G', 'genotype'
	) RETURNING id
),
...
WITH config_prediction AS (
	INSERT INTO af.property(
		...
		'catalog_item', 'character varying', 'gxe', 'GxE', 'loc.genotype'
	) RETURNING id
),
config_prediction_property_config_link AS (
	INSERT INTO af.property_config(
